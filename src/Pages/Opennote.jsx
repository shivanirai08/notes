...
const Opennote = ({ note, onClose, onEdit, onDelete, onFavorite , isFavorite }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
      <div
        className="w-full h-1/2 max-w-2xl bg-white dark:bg-zinc-900 rounded-xl shadow-xl relative p-6 overflow-hidden"
        style={{
      ...
      >
        ...
        {/* Title */}
        <h2 className="text-2xl font-bold mb-4 pr-20 break-words">{note.title}</h2>

        {/* Content */}
        <div
          className="prose prose-sm max-h-80 overflow-y-auto text-gray-800 dark:text-gray-900 dark:prose-invert"
          dangerouslySetInnerHTML={{ __html: note.content }}
        />

        {/* Footer */}
        <div className="text-xs text-right text-gray-600 dark:text-gray-800 mt-6 absolute bottom-4 right-4">
          {note.date}
        </div>
      </div>
    </div>
